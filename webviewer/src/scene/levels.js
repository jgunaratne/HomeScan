...
const SLIDERS = new Set(), CLOSED = new Set(), OPEN = new Set();
for (const r of [...(PHOTOS || []), ...(ROOMS || [])]){
  ...
  // centre of the wall they are in; and doorways it wants left bare, no leaf.
  for (const [x, z] of r.finishes?.closedDoors || []) CLOSED.add(x.toFixed(2) + ',' + z.toFixed(2));
  for (const [x, z] of r.finishes?.openDoors || []) OPEN.add(x.toFixed(2) + ',' + z.toFixed(2));
}
...
export function build(L, above){
  ...
  for (const w of holeWalls){
    for (const o of w.holes){
      ...
      if (o.k === 'door') doorLeaf(trim, L, w, o, blockers, SLIDERS, CLOSED, OPEN);
