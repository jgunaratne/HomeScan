...
test('storeys do not borrow from each other',()=>{
...
test('floors merge only across rooms that declare the same finish',()=>{
  const east=room(1,'East bedroom',{floor:'timber'},{floor:[6,'boardsE']});
  const west=room(1,'West bedroom',{floor:'timber'},{floor:[2,'boardsW']});
...
test('a reviewed rectangle scores zero, so it wins its group',()=>{
  const reviewed=room(0,'Great room',{floor:'timber'},{floor:[0,'reviewed']});
  const auto=room(0,'Living room',{floor:'timber'},{floor:[0.5,'automatic']});
  make([reviewed,auto])();
  assert.equal(auto.mats.floor,'reviewed');
});

test('an undressed room contributes nothing and receives nothing',()=>{
  const dressed=room(0,'Great room',{floor:'timber'},{floor:[4,'boards']});
  const undressed={level:0,name:'Garage',finishes:{floor:'timber'},pick:{floor:{score:1}},mats:null};
...
test('the shipped scan declares finishes that group into one ceiling a storey',()=>{
  ...
  const timber=rooms.filter(r=>r.level===1&&r.finishes?.floor==='timber');
  assert.ok(timber.length>=3,'the upstairs bedrooms are what the floor grouping is for');
});
